...
/**
 * Implementation of a consumer group member for the native PostgreSQL queue.
 * 
...
public class PgNativeConsumerGroupMember<T> implements dev.mars.peegeeq.api.messaging.ConsumerGroupMember<T> {
    ...
    public PgNativeConsumerGroupMember(String consumerId, String groupName, String topic,
                                      ...
                                      PgNativeConsumerGroup<T> parentGroup) {
        ...
        logger.debug("Created consumer group member '{}' in group '{}' for topic '{}'", 
    ...
    public void setMessageFilter(Predicate<Message<T>> messageFilter) {
        this.messageFilter = messageFilter;
        logger.debug("Updated message filter for consumer '{}' in group '{}'", consumerId, groupName);
    }
    ...
    public void start() {
        ...
        if (active.compareAndSet(false, true)) {
            ...
            logger.info("Started consumer member '{}' in group '{}' for topic '{}'", 
                consumerId, groupName, topic);
        }
    }
    ...
    public void stop() {
        if (active.compareAndSet(true, false)) {
            logger.info("Stopped consumer member '{}' in group '{}' for topic '{}'", 
                consumerId, groupName, topic);
        }
    }
    ...
    public void close() {
        if (closed.compareAndSet(false, true)) {
            stop();
            logger.debug("Closed consumer member '{}' in group '{}' for topic '{}'", 
                consumerId, groupName, topic);
        }
    }
    ...
    public boolean acceptsMessage(Message<T> message) {
        ...
        try {
            ...
            logger.warn("Error applying message filter for consumer '{}' in group '{}': {}", 
