...
**Status**: Design Specification (with Implementation Status)
...
| ⚠️ | **PARTIAL** | Schema/infrastructure exists but service layer incomplete |
| ❌ | **NOT IMPLEMENTED** | Design only - not yet implemented |

### Quick Status Summary

**✅ IMPLEMENTED (Production Ready):**
- Reference Counting Mode (complete)
- Topic Configuration (QUEUE/PUB_SUB semantics)
- Subscription Management (full lifecycle)
- Consumer Group Fetching (concurrent-safe)
...
| Feature | Status | Schema | Service Layer | Notes |
|---------|--------|--------|---------------|-------|
| **Reference Counting Mode** | ✅ | Complete | Complete | Fully functional |
| Topic Configuration | ✅ | Complete | `TopicConfigService` | Production ready |
| Subscription Management | ✅ | Complete | `SubscriptionManager` | All states implemented |
| Consumer Group Fetching | ✅ | Complete | `ConsumerGroupFetcher` | Concurrent-safe |
...
- ❌ **NOT IMPLEMENTED**: Bitmap tracking optimization
- ❌ **NOT IMPLEMENTED**: Offset/Watermark mode for >16 consumer groups
...
### Mode 1: Reference Counting (Bitmap/Counter) ✅ IMPLEMENTED
...
> **Implementation Status**: ✅ IMPLEMENTED - SQL function `cleanup_completed_outbox_messages()` is production-ready and fanout-aware
...
> **Implementation Status**: ✅ IMPLEMENTED - All core APIs are functional

### Topic Configuration ✅ IMPLEMENTED
...
> **Implementation Status**: ✅ IMPLEMENTED - Core services (`CompletionTracker`, `ConsumerGroupFetcher`, `SubscriptionManager`, `TopicConfigService`, `ZeroSubscriptionValidator`) are production-ready

### Consumer Group Lifecycle Validation ✅ IMPLEMENTED
...
> **Implementation Status**: ❌ NOT IMPLEMENTED - These are test scenarios and decision criteria, not implemented features
...
## Partitioned Consumer Groups (OFFSET_WATERMARK Mode)

> **Status**: DRAFT | **Date**: 2026-04-05

This section provides the complete design for the OFFSET_WATERMARK completion tracking mode with partitioned consumption. It extends the overview in [§6 Mode 2](#mode-2-offsetwatermark--not-implemented) with full details on partition keys, assignment, rebalance, consumption flow, and APIs.
...
### 19.5 Consumption Flow

#### 19.5.1 Message Publish
...
#### 19.5.3 Partition Assignment (Rebalance Protocol)
...
#### 19.5.4 Message Fetch (Partitioned, Ordered)
...
#### 19.5.7 Message Status Transition
...
#### 19.5.8 Watermark Calculation and Cleanup
...
```sql
-- Watermark calculation already excludes DEAD subscriptions (§19.5.8 uses subscription_status = 'ACTIVE')
-- But a subscription may die AFTER the watermark was last calculated.

-- Force-advance stale offsets (zombie protection)
UPDATE {schema}.outbox_partition_offsets
SET committed_offset = (
    SELECT watermark_id
    FROM {schema}.outbox_topic_watermarks
    WHERE topic = outbox_partition_offsets.topic
)
WHERE committed_at < NOW() - INTERVAL '7 days'  -- Configurable TTL
...
#### 19.5.10 Forced Retention Cap

**Problem**: A topic with zero ACTIVE subscriptions will never advance the watermark.

**Solution**: Enforce a maximum retention period regardless of subscription state.

```java
public class TopicRetentionConfiguration {
    private final Duration messageRetention;      // Default: 24 hours (normal cleanup)
    private final Duration maxRetention;          // Default: 30 days (forced cap)
    private final Duration zombieSubscriptionTTL; // Default: 7 days (force-advance stale offsets)
}
```

The watermark cleanup job should also drop partitions older than `maxRetention` regardless of watermark position. This prevents a single stalled subscription from pinning data indefinitely.

### 19.6 Rebalance Protocol Detail

#### 19.6.1 Consumer Instance Lifecycle
...
### 19.7 Interaction with Existing Infrastructure
...
### 19.8 Key Design Decisions

| Decision | Rationale |
|----------|----------|
| **Re-use `message_group` as partition key** | Already exists on outbox table, already carried through ConsumerGroupFetcher and OutboxMessage, no schema change needed |
...
#### 19.9.1 Existing API Baseline (v1.1.0)
...
- **API layer** gets OFFSET_WATERMARK mode support in `ConsumerGroup.start(SubscriptionOptions)` and `PgNativeQueueConsumer`

#### 19.9.2 SubscriptionService Additions
...
- Rebalance listener callback (`onPartitionsRevoked` / `onPartitionsAssigned`) for application coordination
- Tests: multi-instance consumption with guaranteed ordering, setMessageHandler with partitioned topic, rebalance during processing

### 19.11 Open Questions (Partitioned Mode)
...
5. **Consumer group migration**: Can an existing REFERENCE_COUNTING topic be migrated to OFFSET_WATERMARK? What happens to in-flight `outbox_consumer_groups` rows during migration? (See §19.13 for proposed migration steps.)

### 19.12 Pre-GA Decision Checkpoints (Partitioned Mode)

Mandatory load tests and chaos engineering scenarios that must pass before declaring OFFSET_WATERMARK mode production-ready.

#### 19.12.1 Load Test Matrix

| Test ID | Consumer Groups | Msg/Sec | Duration | Success Criteria |
|---------|-----------------|---------|----------|------------------|
| **LT-5** | 16 | 10,000 | 1 hour | All thresholds met |
| **LT-6** | 64 | 10,000 | 1 hour | All thresholds met |
| **LT-7** | 100 | 30,000 | 1 hour | All thresholds met |

#### 19.12.2 Success Thresholds

| Metric | Pass | Fail | Measurement |
|--------|------|------|-------------|
| **Table bloat** | `bloat_pct < 20%` | `>= 20%` | `pg_stat_user_tables.n_dead_tup` |
| **WAL rate** | < 50% of disk I/O capacity | >= 50% | `pg_stat_wal` |
| **Cleanup throughput** | > production rate (msg/sec) | <= production rate | Watermark advance rate |
| **Consumer fetch p95** | < 10ms | >= 10ms | `pg_stat_statements` |

#### 19.12.3 Kill-and-Resurrect Chaos Test
...
**Success criteria:**
- Dead detection within 5 minutes of last heartbeat
- Watermark continues advancing (not pinned by dead group)
- Other groups never blocked
- Resurrected group resumes from its last committed offset, not from zero
- Steady state recovery within 30 minutes

#### 19.12.4 Partition Management Test

```
Setup: Daily partitions, 3 consumer groups with different processing rates, 7-day retention
Day 1-10: Run at 10,000 msg/sec
Day 8:    Verify Day 1 partition dropped (7-day retention)
Day 9:    Kill one consumer group (test zombie protection)
Day 10:   Verify watermark excludes dead group, partition still drops
```

**Success criteria:**
- Partitions created automatically
- Partitions dropped when watermark advances past retention
- Zombie subscriptions do not pin partitions (force-advance after TTL)

#### 19.12.5 Decision Matrix

| Test | Blocker for GA? | Action if Failed |
|------|-----------------|------------------|
| **LT-5/6/7 Load Tests** | YES | Fix watermark calculation or offset commit performance |
| **Kill-and-Resurrect** | YES | Fix dead detection or partition reassignment |
| **Partition Management** | YES | Fix watermark cleanup or zombie protection |

### 19.13 Migration from Reference Counting to Offset/Watermark

Migration requires schema additions, data backfill, code switch, and cleanup.

#### 19.13.1 When to Migrate

| Current State | Action |
|---------------|--------|
| ≤ 8 consumer groups, low throughput | Stay on Reference Counting |
| 8-16 groups, moderate throughput | Monitor bloat; migrate if thresholds exceeded |
| > 16 groups OR > 10,000 msg/sec | Migrate to Offset/Watermark |

#### 19.13.2 Migration Steps

**Step 1: Create new schema** (Phase 1 migration, no downtime)

```sql
-- Tables from §19.4.1: outbox_partition_assignments, outbox_partition_offsets, outbox_topic_watermarks
-- Index from §19.4.3: idx_outbox_topic_msggroup_id
```

**Step 2: Backfill offsets from current state**

```sql
INSERT INTO {schema}.outbox_partition_offsets (topic, group_name, partition_key, committed_offset)
SELECT s.topic, s.group_name,
       COALESCE(o.message_group, '__default__') AS partition_key,
       COALESCE(MAX(cg.message_id), 0) AS committed_offset
FROM {schema}.outbox_topic_subscriptions s
CROSS JOIN (
    SELECT DISTINCT COALESCE(message_group, '__default__') AS message_group
    FROM {schema}.outbox
    WHERE topic = s.topic
) o
LEFT JOIN {schema}.outbox_consumer_groups cg
    ON cg.group_name = s.group_name
    AND cg.message_id IN (
        SELECT id FROM {schema}.outbox
        WHERE topic = s.topic
          AND COALESCE(message_group, '__default__') = o.message_group
    )
WHERE s.subscription_status = 'ACTIVE'
GROUP BY s.topic, s.group_name, o.message_group;
```

**Step 3: Initialize watermarks**

```sql
INSERT INTO {schema}.outbox_topic_watermarks (topic, watermark_id)
SELECT topic, MIN(committed_offset)
FROM {schema}.outbox_partition_offsets
GROUP BY topic;
```

**Step 4: Switch topic to OFFSET_WATERMARK mode** (brief pause in processing)

```sql
UPDATE {schema}.outbox_topics
SET completion_tracking_mode = 'OFFSET_WATERMARK'
WHERE topic = $1;
```

**Step 5: Deploy code** that routes OFFSET_WATERMARK topics to `PartitionedFetcher` instead of `ConsumerGroupFetcher`.

**Step 6: Cleanup old data** (after verification period)

```sql
-- Remove outbox_consumer_groups rows for migrated topics
DELETE FROM {schema}.outbox_consumer_groups
WHERE message_id IN (
    SELECT id FROM {schema}.outbox WHERE topic = $1
);
```

#### 19.13.3 Downtime

Steps 1-3: No downtime (additive schema changes).
Step 4: Brief pause (~seconds) to switch mode.
Step 5: Rolling deploy.
Step 6: Background cleanup after verification.

#### 19.13.4 Rollback

If issues arise after Step 4, revert `completion_tracking_mode` to `'REFERENCE_COUNTING'`. The `outbox_consumer_groups` data is still intact until Step 6.
