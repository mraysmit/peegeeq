...
| [ ] | [OutboxConsumerGroupGracefulShutdownTest.java](../../peegeeq-outbox/src/test/java/dev/mars/peegeeq/outbox/OutboxConsumerGroupGracefulShutdownTest.java) | T4, T7 | — | T7: L94, L122 bare `group.close()` still present; T4: L87 `manager.start().await()`, L135, L154, L173, L190 `stopGracefully().await()`, L170, L200 `group.start().await()` |
...
| [ ] | [OutboxConsumerGroupReviewFixesTest.java](../../peegeeq-outbox/src/test/java/dev/mars/peegeeq/outbox/OutboxConsumerGroupReviewFixesTest.java) | T7 | — | T7: L95, 432, 492, 582 bare `group.close()` still present; L509 already fixed with `.onFailure()`. |
