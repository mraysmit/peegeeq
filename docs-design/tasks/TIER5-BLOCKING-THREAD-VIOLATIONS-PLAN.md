...
| [x] | [examples/PerformanceComparisonExampleTest.java](../../peegeeq-native/src/test/java/dev/mars/peegeeq/pgqueue/examples/PerformanceComparisonExampleTest.java) | T4, T7 | — | Fixed 2026-05-28: T4 — `allProcessed.future().await()` in `testConfiguration` helper replaced with `AtomicBoolean` + `CountDownLatch.await(35s)` pattern. T7 — `nativeFactory.close()` (QueueFactory returns Future<Void>) was discarded bare in tearDown; now chained via `(nativeFactory != null ? nativeFactory.close() : ...).eventually(manager.closeReactive)`. |
