...
This document provides a comprehensive guide to PeeGeeQ's internal architecture and message flow. Here are the key points:

### **Architecture Overview**
- **Hexagonal/Ports & Adapters** pattern with strict layered separation
- **Vert.x 5.x-native** reactive application using `Future<T>` as primary API
- **7 core modules**: api (contracts), db (database layer), native/outbox/bitemporal (adapters), runtime (composition), rest (HTTP layer), management-ui (React frontend)

### **Key Architectural Principles**
1. **peegeeq-api** - Pure contracts only (interfaces, DTOs, configs) with NO implementations
2. **peegeeq-runtime** - Composition root that wires all implementations together via `DatabaseSetupService` facade
3. **peegeeq-rest** - Thin HTTP adapter exposing runtime services over REST/SSE
4. **peegeeq-management-ui** - React/TypeScript UI consuming REST API via HTTP
...
- **Bi-temporal Event Store**: Append, corrections, temporal queries, point-in-time queries, versioning, SSE streaming
- **Consumer Groups**: Create, join, leave, list with real PostgreSQL-backed implementation
- **Dead Letter Queue**: List, reprocess, delete, stats, cleanup
- **Health Checks**: Overall, component-level health monitoring
- **Webhooks**: Push-based delivery with retry logic

### **Dependency Rules**
Strict compile-time dependency constraints prevent circular dependencies and ensure clean architecture (e.g., rest layer cannot depend on db/native/outbox/bitemporal directly).

The document serves as both a developer onboarding guide and a complete traceability matrix from REST endpoints through to database operations.
...
**Implementation Notes (December 2025):** All REST endpoints are fully implemented:
...
| `getRealAggregateCount()` | ✅ **IMPLEMENTED** | Calls `EventStore.getStats().getUniqueAggregateCount()` |
| `getRealMessages()` | ✅ **IMPLEMENTED** | Uses `QueueFactory.createBrowser()` with `browser.browse(limit, offset)` |
| `getRecentActivity()` | ✅ **IMPLEMENTED** | Queries recent events from all active event stores using `EventQuery` with `transactionTimeRange` |
