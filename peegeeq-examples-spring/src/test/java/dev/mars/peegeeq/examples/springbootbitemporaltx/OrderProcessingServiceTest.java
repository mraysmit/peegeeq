...
class OrderProcessingServiceTest {
     ...
     */
    @Test
    void testSingleProductOrderProcessing(Vertx vertx, VertxTestContext testContext) {
                                ...
                                return paymentEventStore.query(EventQuery.builder()
                                    ...
                                    .compose(paymentEvents -> {
                                        ...
                                        return auditEventStore.query(EventQuery.builder()
                                            .aggregateId(orderId)
                                            .build())
                                            .compose(initialAuditEvents -> {
                                                // DEBUG: Log audit events to understand what's being returned
                                                logger.info("Found {} audit events for orderId: {}", initialAuditEvents.size(), orderId);
                                                for (int i = 0; i < initialAuditEvents.size(); i++) {
                                                    BiTemporalEvent<AuditEvent> event = initialAuditEvents.get(i);
                                                ...
                                                if (initialAuditEvents.stream().noneMatch(e -> "TransactionStarted".equals(e.getEventType()))) {
                                                    logger.info("No TransactionStarted events found with aggregateId={}, trying correlationId query", orderId);
                                                    return auditEventStore.query(EventQuery.builder()
                                                        .correlationId(result.getCorrelationId())
                                                        .build())
                                                        .map(correlationEvents -> {
                                                            logger.info("Found {} audit events for correlationId: {}", correlationEvents.size(), result.getCorrelationId());
                                                            for (int i = 0; i < correlationEvents.size(); i++) {
                                                                BiTemporalEvent<AuditEvent> event = correlationEvents.get(i);
                                                ...
                                                return Future.succeededFuture(initialAuditEvents);
                                            })
                                            .map(auditEvents -> {
                                                ...
                                                logger.info("Order: {}, Correlation: {}, Events: 1 order, 1 inventory, 1 payment, {} audit",
                                                    result.getOrderId(), result.getCorrelationId(), auditEvents.size());
                                                return (Void) null;
                                            });
                                    });
                            });
     ...
     */
    @Test
    void testMultiProductOrderProcessing(Vertx vertx, VertxTestContext testContext) {
