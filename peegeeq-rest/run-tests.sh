...
# PeeGeeQ REST Test Execution Helper Script
...
show_help() {
    echo "PeeGeeQ REST Test Runner"
    echo ""
    echo "Usage: $0 [COMMAND]"
    echo ""
    echo "Commands:"
    echo "  core          Run core tests (default, ~8 seconds)"
    echo "  integration   Run integration tests (~13 seconds)"
    echo "  performance   Run performance tests (~10 seconds)"
    echo "  smoke         Run smoke tests (~5 seconds)"
    echo "  slow          Run slow tests (5+ minutes)"
    echo "  all           Run all tests except flaky (~30 seconds)"
...
case "${1:-core}" in
    "core")
        ...
        ;;
    "integration")
        run_tests "integration-tests" "Integration Tests (TestContainers + REST)"
        ;;
    "performance")
