...
class MultiConfigurationIntegrationTest {
    ...
    void setUp() {
        ...
        configManager.registerConfiguration("high-throughput", "development");
        configManager.registerConfiguration("low-latency", "development");
        configManager.registerConfiguration("reliable", "development");
    ...
    void testMultipleQueueConfigurationsInSameApplication() throws Exception {
        ...
        // Create different queue factories for different use cases
        QueueFactory batchProcessingQueue = configManager.createFactory("high-throughput", "native");
    ...
    void testConfigurationBuilderIntegration() throws Exception {
        ...
        QueueFactory highThroughputQueue = QueueConfigurationBuilder.createHighThroughputQueue(
            configManager.getDatabaseService("development"));
        QueueFactory lowLatencyQueue = QueueConfigurationBuilder.createLowLatencyQueue(
            configManager.getDatabaseService("development"));
        QueueFactory reliableQueue = QueueConfigurationBuilder.createReliableQueue(
            configManager.getDatabaseService("development"));
        
        // Verify all queues are healthy
        assertTrue(highThroughputQueue.isHealthy());
        assertTrue(lowLatencyQueue.isHealthy());
        assertTrue(reliableQueue.isHealthy());
        
        // Test that they have different implementation types as expected
        assertEquals("native", highThroughputQueue.getImplementationType());
        assertEquals("native", lowLatencyQueue.getImplementationType());
        assertEquals("outbox", reliableQueue.getImplementationType());
        
        // Clean up
        highThroughputQueue.close();
        lowLatencyQueue.close();
        reliableQueue.close();
    }
    ...
    void testConcurrentMultiConfigurationUsage() throws Exception {
        ...
        // Create multiple queue factories concurrently
        QueueFactory[] factories = new QueueFactory[4];
        factories[0] = configManager.createFactory("high-throughput", "native");
        factories[1] = configManager.createFactory("low-latency", "native");
        factories[2] = configManager.createFactory("reliable", "outbox");
        factories[3] = configManager.createFactory("development", "native");
        
        // Test concurrent message processing
        CountDownLatch latch = new CountDownLatch(40); // 10 messages per factory
        ...
        assertEquals(40, totalProcessed.get(), "Expected 40 messages to be processed");
