 ...
 * If peegeeq-management-ui e2e tests have already run, the container and
 * backend will already be up — this setup detects that and skips startup.
 *
 * Manual cleanup is required (docker stop) when you're done testing.
 */

// Store container info in a file so teardown can access it
// Using process.cwd() which points to peegeeq-utilities-ui when running tests
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // dev server origin (port 3001 for utilities-ui).
        console.log('   Verifying backend CORS configuration for http://localhost:3001...')
        const corsOk = await verifyCors(`${API_BASE_URL}/health`, 'http://localhost:3001')
        if (corsOk) {
    ...
    if (backendAlreadyRunning) {
      ...
      // Project root is one level above peegeeq-utilities-ui (process.cwd())
    ...
    // The backend passes its health check before its setup registry is fully loaded from the DB,
    // so the first GET /api/v1/setups call may return [] even though setups exist.
    // Strategy: if the first call returns empty, wait 3 seconds and retry once to let the
    // registry finish loading, then delete whatever is found.
    console.log('\nCleaning up existing database setups...')
    const fetchSetupIds = async (): Promise<string[]> => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/v1/setups`, {
          method: 'GET',
        ...
        return Array.isArray(data)
          ? data
          : (data.setupIds && Array.isArray(data.setupIds))
            ? data.setupIds
            : []
      } catch {
        return []
    ...
    try {
      let setupIds = await fetchSetupIds()
      if (setupIds.length === 0) {
        // Backend registry may not be fully loaded yet — wait and retry once
        await new Promise(resolve => setTimeout(resolve, 3000))
        setupIds = await fetchSetupIds()
      }
